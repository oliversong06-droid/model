...
with st.sidebar.expander("3. Biology & Chemistry", expanded=True):
    k_consume = st.slider("DO Consumption Rate (k_consume)", 0.01, 0.5, 0.05)
    k_reaer = st.slider("Reaeration Rate (k_reaer)", 0.001, 0.1, 0.01)
    sens_coeff = st.slider("Plankton Sensitivity", 0.0001, 0.001, 0.0001, format="%.4f")
...
if st.sidebar.button("🚀 RUN SIMULATION"):
    ...
    # --- STEP 1: PHYSICS ---
    st.markdown("#### 1. Physics Simulation")
    ...
    # --- STEP 2: AI TRAINING ---
    st.markdown("#### 2. AI Model Training")
    ...
    for epoch in range(num_epochs):
        ...
        # Update Chart
        loss_chart.line_chart(train_losses)
    ...
    DO_next = update_DO(DO_current, DO_sat, pred_oil * 10, k_consume=k_consume, k_reaer=k_reaer, dt=dt)
    Plankton_next = plankton_response(Plankton_current, pred_oil * 10, sens_coeff=sens_coeff, dt=dt)
    ...
    with col1:
        st.metric("Max Oil Conc.", f"{max_oil:.4f}")
    with col2:
...
with st.sidebar:
    ...
    if len(st.session_state.history) > 0:
        ...
        selected_run_label = st.selectbox("Select Past Run", ["Current Run"] + history_labels)
        
        if selected_run_label != "Current Run":
            # Find the selected entry
            selected_idx = history_labels.index(selected_run_label)
            # Adjust index because we reversed the labels
            original_idx = len(st.session_state.history) - 1 - selected_idx
            selected_entry = st.session_state.history[original_idx]
            
            if st.button("Load Past Analysis"):
                st.session_state.selected_run = selected_entry
                st.rerun()
