...
def main():
    ...
    # 2. [중요] 정화 모델을 위한 데이터 파일 저장
    # (반드시 main 함수 안에서, preds 변수가 있을 때 실행되어야 합니다)
    output_path = "taean_prediction_output.npy"
    np.save(output_path, preds.squeeze().cpu().numpy())
    print(f"[Link] 정화 모델용 데이터 저장 완료: {os.path.abspath(output_path)}")
