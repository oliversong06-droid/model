...
def ecological_recovery_index(
    ...
    # Strict 1.0 cap ensures that an over-saturated component (e.g., Algal bloom)
    # doesn't artificially inflate the score of a damaged ecosystem.
    
    DO_rel = np.clip(np.asarray(DO) / (DO_ref + 1e-12), 0.0, 1.0)
    plank_rel = np.clip(np.asarray(plankton) / (plankton_ref + 1e-12), 0.0, 1.0)
    benth_rel = np.clip(np.asarray(benthos) / (benthos_ref + 1e-12), 0.0, 1.0)
